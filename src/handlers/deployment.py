...
class Deployment(ResourceHandler):
    ...
    def _get_resource_body(self) -> client.V1Deployment:
        image = self.spec.get("image", self.defaults.get("odooImage", "odoo:19.0"))

        volumes, volume_mounts = self.get_volumes_and_mounts()

        # Add git-sync sidecar volumes and mounts if addons configured
        sidecar_containers = []
        if hasattr(self.handler, 'addon_sync') and self.handler.addon_sync.has_addons():
            addon_sync = self.handler.addon_sync
            volumes.extend(addon_sync.get_volumes())
            volume_mounts.extend(addon_sync.get_volume_mounts())
            sidecar_containers = addon_sync.get_sidecar_containers()
        ...
        main_container = client.V1Container(
            ...
            resources=self.spec.get(
                "resources",
                self.defaults.get("resources", self._get_default_resources()),
            ),
        ...
        all_containers = [main_container] + sidecar_containers
    ...
    def _get_default_resources(self) -> dict:
        """Get default resources for 4-worker Odoo setup."""
        return {
    ...
    def get_environment_variables(self) -> List[client.V1EnvVar]:
        ...
        # Check if using CNPG mode
        use_cnpg = hasattr(self.handler, '_use_cnpg') and self.handler._use_cnpg

        if use_cnpg:
            # CNPG mode: get connection from CNPG cluster secret
            cnpg_secret = f"{self.name}-pg-app"
            env_vars = [
                client.V1EnvVar(
                    name="HOST",
                    value=f"{self.name}-pg-rw",
                ),
                client.V1EnvVar(
                    name="PORT",
                    value="5432",
    ...
    def get_volumes_and_mounts(
        ...
        """Get volumes and mounts based on storage mode."""
        use_cnpg = hasattr(self.handler, '_use_cnpg') and self.handler._use_cnpg

        if use_cnpg:
            # CNPG mode: use JuiceFS PVC for filestore
            return get_cnpg_volumes_and_mounts(self.name)
        else:
            # Legacy mode: regular PVC
...
def get_cnpg_volumes_and_mounts(
    instance_name: str,
) -> Tuple[List[client.V1Volume], List[client.V1VolumeMount]]:
    """
    Get volumes and mounts for CNPG mode with JuiceFS filestore.

    Uses JuiceFS PVC for filestore instead of regular PVC.
    """
    volumes = [
        client.V1Volume(
            name="filestore",
            persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource(
                claim_name=f"{instance_name}-filestore"
            ),
        ),
        client.V1Volume(
            name="odoo-conf",
