...
from .cnpg_cluster import CNPGCluster
from .juicefs_storage import JuiceFSStorage
from .addon_sync import AddonSync
...
class OdooHandler(ResourceHandler):
    def __init__(self, body=None, **kwargs):
        ...
        # CNPG cluster must be created before JuiceFS (JuiceFS needs DB connection)
        self.cnpg_cluster = CNPGCluster(self)
        self.juicefs_storage = JuiceFSStorage(self)

        self.odoo_user_secret = OdooUserSecret(self)

        # Addon sync manages git-sync sidecars (not a ResourceHandler, used by Deployment)
        self.addon_sync = AddonSync(self)
        ...
        if self._use_cnpg:
            # New mode: CNPG + JuiceFS (no legacy filestore_pvc)
            self.handlers = [
                self.pull_secret,
                self.cnpg_cluster,
                self.juicefs_storage,
            ...
            # Legacy mode: external DB + PVC filestore
